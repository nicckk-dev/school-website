...
export const CommonEasUpload = (props) => {
    ...
    const formatvalidation = (val) => {
        let regex = /^([a-zA-Z0-9\s_\\.\-:\)\(\(.*?)])+(.pdf|.PDF|.jpeg|.jpg|.png|.bmp|.xls|.xlsx|.xlsb|.csv|.doc|.docx)$/;
        ...
        if ((limit && info?.fileList?.length <= limit) || !limit) {
            if ((file.size <= fileSize) || !fileSize) {
                if (formatvalidation(file?.name)) {
                    ...
                    dispatch(messageSerivce({ messageType: "error", messageContent: 'This File Format Is Not Allowed' }))
                }
            }
            else {
            ...
            <div className='Modalscroll' style={{ height: '110px' }}>
                <div className='modalimgList'>
                    {fileList.map((attachment, index) => (
                        <div className={`singleimg cursor-pointer ${(fileObjRef.current?.imG_URL == attachment.imG_URL) ? 'singleimg_active' : ""}`}
                            style={{ height: '100px' }}
                            onClick={() => handlePreview(attachment)} key={index}
                        >
                            <div className={`me-1 text-end cursor ${readOnlyFlg != 'readonly' ? 'd-block' : 'd-none'}`}
                                onClick={(e) => {
    ...
    return (
        <>
            ...
            >
                {
                    (isgetBolbDataFetch || UploadFetching || uploadLoader || SasWiseUploadFetching) && <ModalLoader />
                }
                {previewImage ?
                    (
                        <div>
            ...
            <PDFViewer
                ...
                isModalVisible={pdfViewer} filename={filename}
